...
produced by 454/Roche (e.g. @Sogin2006), Illumina
(e.g. @Gloor2010), and IonTorrent (e.g. @Junemann2012). It was
once common to sequence fewer than 100 16S rRNA gene sequences from several
samples using the Sanger approach (e.g. @Mccaig1999). Now it is common to
generate thousands of sequences from each of several hundred samples
[@HMP2012]. The advance in throughput has
...
algorithms [@Kozich2013; @Schloss2011a]. There has been a tradeoff
...
Titanium platforms the raw error rate varies between 1 and 2%
[@Kozich2013; @Schloss2011a]. Yet, it was possible to obtain error rates below
...
including limited ability to identify chimeras [@Haas2011; @Edgar2011]
and inflation of alpha- and beta-diversity metrics [@Huse2010; @Kunin2010; @Kozich2013; @Schloss2011a].
...
[@Liu2008; @Werner2011; @Wang2007]. There is clearly a need to develop
sequencing technologies that will allow researchers to generate high quality
full-length 16S rRNA gene sequences in a high throughput manner.
...
research domain @Mosher2013; @Mosher2014; @Fichot2013].
...
multiple times [@Au2012]. According to Pacific Biosciences the platform
...
sequencing 16S rRNA genes [@Fichot2013; @Mosher2014; @Mosher2013].
Previous studies utilizing the technology have removed reads with mismatched
primers and barcodes, ambiguous base  calls, and low quality scores [@Fichot2013].
Others have utilized the platform without describing the bioinformatic pipeline
that was utilized [@Mosher2014; @Mosher2013]. Regardless of the
...
sequencing strategy [@Kozich2013].  Briefly, we used a “Mock Community”
...
pooling were described previously [@Kozich2013]. The SMRTbell adapters
...
All sequencing data were curated using mothur [@Schloss2009a] and analyzed
using the R programming language [@R2016]. The raw data can be
...
was performed using the seq.error command in mothur [@Schloss2011a]. *De
novo* chimera detection was also performed on the mock and other sequence data
using the abundance-based algorithm implemented in UCHIME [@Edgar2011].
Sequences sequences were aligned against a SILVA-based reference alignment
[@Pruesse2007] using a profile-based aligner [@Schloss2009b] and
were classified against the SILVA [@Pruesse2007], RDP [@Cole2013],
and greengenes [@Werner2011] reference taxonomies  using a naive Bayesian
classifier [@Wang2007]. Sequences were assigned to operational taxonomic
units using the average neighbor clustering algorithm with a 3% distance
threshold [@Schloss2011b].  Detailed methods including this paper as an R
markdown file are available as a public online repository
(http://github.com/SchlossLab/Schloss_PacBio16S_PeerJ_2016).
...
to sequence various regions within the 16S rRNA gene.** The predicted error rate using PacBio's sequence analysis algorithm correlated well with the observed error rate (Pearson's R: -0.67; A). Because of the large number of sequences, we randomly selected 5% of the data to show in panel A. The sequencing error rate of the amplified gene fragments
...
**Figure 4. Percentage of 1-nt variants that occured up to ten times.** Sequences that were 1 nt different from the mock community reference sequences were counted to determine the number of times each variant appeared by region within the 16S rRNA gene.
